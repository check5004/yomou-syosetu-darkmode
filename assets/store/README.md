...
最初に、作品名・作者名・あらすじ・章やエピソードの見出しが描画される範囲を取得し、その文字の範囲にCSSのぼかしを重ねて撮影します。次に、サイトのロゴ・サイト名・固有の文言・アイコンを、完全に不透明な灰色（`#737b85`）の矩形で覆います。ロゴにはぼかしやモザイクを使わず、全体を図形で隠しています。その他の検索条件やボタン、レイアウトは残します。

元HTML・CSSと未加工画像はGit管理対象外で保管し、このフォルダーでは文字のぼかしと図形の追加を済ませた画像だけを公開します。各画像は1280×800です。

- 図形追加前の文字ぼかし工程：[有効状態の検証データ](screenshots/verification.json)・[無効状態と比較の検証データ](screenshots/before-after-verification.json)。この工程で、ぼかし範囲外の画素が変わっていないことを確認しています。
- 最終画像の図形追加工程：[図形の検証データ](screenshots/branding-verification.json)。最終画像のSHA-256、図形の座標、不透明度、図形範囲外の変更画素数を記録します。

無効状態は拡張機能の設定を「オフ」にして撮影します。反転や画像の色変換は行わず、サイト本来のCSSによる表示を使います。作品の文字をぼかす範囲、追加した図形の位置、スクロール位置は有効・無効でそろえています。ぼかし・図形の追加・撮影時の広告省略は掲載画像だけの処理で、拡張機能の機能ではありません。
